...
class HeaderAPIView(APIView):
    def get(self, request, lang=None):
        ...
        plan_headerSR = PlanHeaderSerializer(plan_header, many=True, context={'lang': lang})
        stepsSR = StepSerializer(steps, many=True, context={'lang': lang})
        projects = Project.objects.all()
        projects_header = ProjectsHeader.objects.first()
        projectsSR = ProjectSerializer(projects, many=True, context={'lang': lang})
        projects_headerSR = ProjectHeaderSerializer(projects_header, many=True, context={'lang': lang})
        ps_header = PSHeader.objects.first()
        problems = Problem.objects.all()
        solutions = Solution.objects.all()
        ps_headerSr = PSHeaderSerializer(ps_header, many=True, context={'lang': lang})
        problemsSR = ProblemSerializer(problems, many=True, context={'lang': lang})
        solutionsSR = SolutionSerializer(solutions, many=True, context={'lang': lang})
        services_header = ServicesHeader.objects.first()
        services = Service.objects.all()
        services_headerSR = ServicesHeaderSerializer(services_header, many=True, context={'lang': lang})
